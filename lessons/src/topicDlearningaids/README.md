...
| **Inheritence** | An OOP feature. A relationship between two classes whereby a child class "is-a" type of parent class e.g., a `Lion` class "is-a" type of `Cat` class. A child class will inherit data and actions of the parent class and is able to access parent traits that are `protected`. This is easy to understand and implement using the `extends` keyword e.g., `Lion extends Cat` but it is considered to be a tight dependency and such a feature should not be overused. A class can only `extend` one class! |
